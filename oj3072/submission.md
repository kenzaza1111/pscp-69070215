...
```text
-รับ input text  lowtext(ทำให้เป็นตัวเล็ก) lentext(จำนวน text) letterlist เเละ countlist ไว้ check
- เช็คว่าความยาวข้อความอยู่ในช่วงที่โจทย์กำหนด
- วนลูปดูตัวอักษรทีละตัว ถ้าตัวนั้นเป็นสระให้ เข้า if else เช็คว่าเป็นสระตัวไหน แล้วบวกค่าใน index ที่ตรงกันของ countlist +1 ถ้า ไม่ให้ continew

- for loop 5 times ถ้าสระตัวนั้นถูกนับได้มากกว่า 0 ครั้ง ก็ print else ก็ไม่ print
```
...
```text
สร้าง list check สระ เเลว้ใช้ loop check ทีละตัว เเล้ว +1 เข้าไปในตำเเหน่ง list[0]*5
```
...
```text
ถ้าวิธีสุดท้ายเหมือนกับแผนแรก สร้าง list check สระ เเล้วใช้ loop check ทีละตัว
```
