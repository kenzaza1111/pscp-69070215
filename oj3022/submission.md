...
```text
- สร้าง functions main()
- รับค่สา input num(องศาของtemp) / temis(คือ หน่วยของ num) / tempwant(คือ tempที่ต้องการเปลี่ยน)
- list text ที่สามารถใส่ได้ในตัวแปร templist
- เข้า if else แรก โดย check ก่อนว่า tempis and tempwant อยุ่ใน list ของ templist หรือเปล่า
- สร้าง if else ข้างในอีกที โดยเราจะแปลงค่าที่ user input(tempis) เข้ามาแล้วคำนวนจากหน่วย temp นั้น ให้เป็น kelvin 
- จากนั้นเมื่อเราได้ k มาแล้ววให้นำ k ไปเข้าสูตร ขอแต่ละ หน่วยtemp(tempwant)  แล้วแปลงให้เป็น temp ของหน่วยนั้น โดยยึดจาก kelvin เป็นหลัก ถ้า tempwant = kevlinb ก็ไม่ต้องคำนวน
- ก่อน print result ให้ ตัดจุดทศนิยมให้เหลือแค่ 2 ตำเเหน่ง  
```
...
```text
เราต้องวิธีการย่อ if else ให้น้อยที่สุด เนื่องจาก pep-8 
แล้วสมไก่รดาสูตรในการคำนวนจาก tempis เป็น k เล้ว check  tempwant โดยใช้ K ที่เราคำนวนได้มาใช้ในการคำนวน
```
...
```text
เป็น case  พิเศษ โดย output จะออกมาเป็น -
```
