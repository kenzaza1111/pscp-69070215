...
```text
-รับ str input เเล้ว split ให้เป็น list เเล้วกำหนดตัวเเปรว่า first คือตัวเเรก sec คือตัวท้าย
เเล้วกำหนด list ว่าง
- for loop ตั้งเเต่ 1 ถึงตัวท้าย +1
- check if เเรก ว่า ถ้า i < 2 (ถ้าเป็น 1 or 0 ให้ continew ไปเลย) ถ้าไม่เข้าเงื่อนไข จะกำหนดตัวเเปร t = 0 มาเพื่อ check จำนวนของจำนวนเฉพาะ
- for loop ตั้งเเต่ 2-i ถ้า i หาร x ไม่ลงตัว ให้ t +1 ถ้าหารลงตัว(เเเสดงว่าไมใช่จำนวนเฉพาะ)ให้ continue ข้ามไป
- หลังตรวจครบถ้า ไม่มีจำนวนเฉพาะเลยเเปลว่า i ต้องเป็นจำนวนเฉพาะ
- จานนั้นเก็บจำนวนจำนวนเฉพาะไว้ใน list count จากนั้น check ถ้า count เป็น 0 ให้ พิมพ์แค่บรรทัดสรุป ถ้าไม่ ให้print ทุกตัวใน count เเล้วค่อย print บรรทัดสรุป
```
...
```text
สร้าง for loop เเล้ว check if else โดยใช้ % check เเล้วเก็บไว้ใน list
```
...
```text
คล้ายเเผมเเรกเเต่จะ check ว่า จำนวนเฉพาะ ติดลบไม่ได้จึงเพิ่มเงื่อนไข if i < 2: ให้ continue
```
...
```text
เป็น testcase ที่ไม่มีจำนวนเฉพาะ
```
