...
```text
-สร้าง function main()
-รับ input N << ตัวเลขเริ่มจากช่องไหน
-สังเกตจาก pattern ของ ตัวอย่างจะรู้ว่า จำนวนประตูที่ต้องเปิดในตัวเลขสุดท้ายของแต่ละแถว จะเป็นค่าที่มากที่สุดของแถววนั้นแล้วตัวที่ n-1 (ตัวก่อนหน้า) จะมีค่าลดลง1 สลับกันไปแล้ววตัวท้ายสุดของแต่ละะแถวจะมีค่า +2 ไปเรื่อยๆ
- สังเกตุตัวเเถว แถว 1 เป็นเลขคี่ 2คู่ 3คี่ สลับกันไป
-หาว่า input ที่เรา input มาอยู่แถววไหนโดยจะถอดรากที่2 แล้ววปัดขึ้น
-looped ตั้งแต่ 1 - แถว(lines)
-เพิ่มค่า answer looped + 2
- เมื่อ looped เสร็จให้ check ค่า line ว่า อยุ่ในแถววที่เป็นเลขคู่ or คี่
-ถ้าเป็นคี่ ให้ check อีกว่า ถ้าอยุ่เเถวคี่เเล้ว N เป็นเลขคี่ ให้ answer -=1
ถ้าอยุ่แถวคู่แล้ว N เป็นคี่ answer -=1
- print(answer)
```
...
```text
หา pattern ของการคำนวนประตูให้ได้ก่อน
```
...
```text
Algorithm เหมื่อนในข้อ 2 เลยครับ ทำการหา patten ก่อนแล้วค่อย แปลงเป็น code
```
