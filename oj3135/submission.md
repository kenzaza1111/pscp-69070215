...
```text
-รับ input text  เเล้ว spilt เก็บเป็นตัวเเปร alls(จำนวนคนทั้งหมด) another(ส่งต่อไปอีกกี่คน) boom(ขโมย)
เเละ กำหนด ตัวเเปร now(ตำแหน่งคนที่ถือของขวัญตอนนี้) =1 เเละ answer(จำนวนคนที่ได้(คำตอบ)) = 1

- เข้า if boom == 1(ถ้าขโมยคือคนที่ 1) >>ให้จบการทำงานเลย
ถ้าผ่าน if เเรกมา ให้ now += another ส่งต่อไปอีก another คน 
ถ้าเลยคนสุดท้าย (now > alls) ให้วนกลับต้นวง

- if now == boom (ถึงมือขโมย) ให้answer+1 เเล้วจบการทำงาน
  if now == 1 (วนกลับมาที่คนแรก) ให้จบการทำงาน
  ถ้าไม่โดนทั้ง 2 กรฌีให้ answer += 1 เเล้วทำวนต่อไป

- พอจบการทำงานเเล้วให้ print ค่า  answer ออกมา
```
...
```text
รับ input มา เเล้ว looped ตามจำนวนคนทั้งหมด(เป็นวิธีที่ผิด)
```
...
```text
เหมื่อนกับ Section 2 โดยการ check เป็น while จนกว่าจะ เป็น false (check ว่าถึงมือโจร/ถึงมือคนเเรกไหม) เเล้วก็ค่าไว้ในตัวเเปร answer เเล้ว print answer
```
