...
```text
- รับข้อความเข้ามา 5 บรรทัด
- เก็บข้อความแต่ละบรรทัดไว้ใน textlist
- ต้องหาว่าข้อความบรรทัดไหนยาวที่สุดเพื่อเอาไปกำหนดขนาดกรอบ
- ด้านบนและด้านล่างของกรอบใช้ *
- ด้านซ้ายและด้านขวาของข้อความมี * และมี ช่องว่าง คั่น 1 ช่อง
- ถ้าข้อความสั้นกว่าข้อความที่ยาวที่สุด ต้องเติม ช่องว่าง ด้านหลังให้เท่ากัน
```
...
```text
สร้าง list ว่างไว้เก็บข้อความ
ใช้ for loop รับข้อความทั้งหมด 5 บรรทัด
แล้วใช้ len() เช็คความยาวของแต่ละข้อความ
เก็บค่าความยาวที่มากที่สุดไว้ใน maxs

หลังจากนั้น print * ด้านบน
แล้วใช้ for loop แสดงข้อความทั้ง 5 บรรทัด
โดยเติม space ให้ข้อความมีความยาวเท่ากัน
แล้ว print * ด้านล่าง
```
...
```text
ใช้ for loop 5 รอบเพื่อรับข้อความ
แต่ละรอบจะใช้ input() รับข้อความแล้วใช้ strip()
จากนั้นเก็บข้อความลงใน textlist

ใช้ if เช็คว่า len(text1) มากกว่า maxs หรือไม่
ถ้ามากกว่าจะเปลี่ยนค่า maxs เป็นความยาวของข้อความนั้น

หลังจากรับครบแล้วใช้ maxs + 4 เพื่อกำหนดความยาวของกรอบ
จากนั้นใช้ for loop แสดงข้อความใน textlist
และเติม space ตามความแตกต่างของความยาวข้อความกับ maxs
สุดท้าย print กรอบด้านล่าง
```
...
```text
ทดสอบกรณีที่ข้อความมีความยาวต่างกันมาก
```
...
```text
ทดสอบกรณีที่ข้อความทั้ง 5 บรรทัดมีความยาวเท่ากัน
```
